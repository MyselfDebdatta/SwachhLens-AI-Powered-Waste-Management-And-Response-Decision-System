    ...
    if os.path.exists(exp_file):
        try:
            ...
            if "Full SwachhLens" in df_agg.index and "Fixed" in df_agg.index:
                comparison = {
                    "AI": {
                        "distance_km": float(df_agg.loc["Full SwachhLens", "Distance_km_mean"]),
                        "fuel_liters": float(df_agg.loc["Full SwachhLens", "Fuel_Liters_mean"]),
                        "overflow_bins": float(df_agg.loc["Full SwachhLens", "Overflow_Events_mean"]),
                        "truck_utilization_pct": float(df_agg.loc["Full SwachhLens", "Utilization_pct_mean"]),
                        "duration_hours": round(float(df_agg.loc["Full SwachhLens", "Distance_km_mean"]) / 30.0, 1)
                    },
                    "Fixed": {
