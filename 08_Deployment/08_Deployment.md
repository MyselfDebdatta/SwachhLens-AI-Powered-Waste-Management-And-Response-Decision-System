...
SwachhLens uses Docker and Docker Compose to ensure a reproducible environment across development and production. The deployment architecture consists of three core containers:
...
services:
  db:
    image: postgres:15
    environment:
      POSTGRES_USER: swachhlens_user
      POSTGRES_PASSWORD: swachhlens_password
  ...
  backend:
    ...
    depends_on:
      - db
    environment:
      - DATABASE_URL=postgresql://swachhlens_user:swachhlens_password@db:5432/waste_management
